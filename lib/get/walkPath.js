    ...
    do {
        ...
        if (next) {
            ...
            if (nextDepth < path.length && nType &&
                ...
                // Fast copy is available.
                if (fromRef) {
                    nextOptimizedPath = fromRef.$_refCopy;
                    nextOptimizedLength = fromRef.$_refCopyLength;
                }

                // ref pointed to undefined, its copied in followReference,
                // so there is no need to worry about copying it again.  Plus
                // missing paths are copied.
                else {
                    nextOptimizedPath = ref[1];
