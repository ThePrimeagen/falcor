...
var __hasValue = require('./../internal/hasValue');

module.exports = function walkPath(model, root, curr, path, depth, seed,
                                   parentSeed, currentSeed, outerResults, requestedPath,
    ...
    var currentVersion = curr[__version];
    var keySet, i, len;
    keySet = path[depth];
    ...
    var hasValue = currentSeed[__hasValue];
    var notAtEndOfPath = nextDepth < path.length;
    var keyIndex = 0;
    var missingKeys, refPath;
    do {
        fromReference = false;
        ...
        var createdValue = walkPath(model, root, next, path, nextDepth, seed,
                            currentSeed, nextSeed, outerResults, requestedPath,
                            nextOptimizedPath, nextOptimizedLength,
                            fromReference);

        // --- Deleting values from the output when building output as we go ---
        ...
        // --- Reporting deref information into the output. ---
        // if hasValue and followed a reference then we need to report a path.
        if (nextSeed && hasValue && followReference) {
            nextSeed[__path] = refPath;
        }

        // If there is not a full path to report then report the key and a
        // pointer to the parent where we can construct the rest of the deref
        // path from.  __parent === null when there is no parent.  There has to
        // be a nextSeed.  It is undefined only when next is not defined.
        else if (nextSeed) {
            nextSeed[__key] = key;
            nextSeed[__parent] = currentSeed;
        }
    ...
    // --- Attaches the version to the current produced value ---
    // If a value is found then on the way out we attach versions to all the
    // branch nodes.
    // TODO: When version is produced with output, then these lines should be
    // uncommented.
    //if (hasValue) {
        //currentSeed[__version] = currentVersion;
    //}
    if (hasValue) {
        currentSeed[__hasValue] = hasValue;
    }
